...
class des():
            ...
            if (offset == 0):
                ...
                while (nxt < 64):
                    if (ref[nxt] != res[nxt]):
                        jn = jn + 1
                    nxt = nxt + 1
                print("Bits Changed = ", jn)
            offset = offset+1
        
        final_res = bit_array_to_string(fix)
    ...
    def generatekeys(self, offset):  # Algorithm that generates all the keys
        self.keys = []
        key = string_to_bit_array(self.password)
        zz=0
        print ("Key ", offset, ": ", ''.join(map(str, key)))
        while zz<offset:
            if key[zz]==0:
                key[zz]=1
            else:
